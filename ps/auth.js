...
const auth = {
    ...
    handleAuthStateChange(session) {
        window.user = session?.user || null;
        const isLoginPage = window.location.pathname.includes('/printsystem/login');

        if (!session && !isLoginPage) {
            window.location.href = '/printsystem/login/';
        } else if (session && isLoginPage) {
            window.location.href = '/printsystem/';
        }
...
window.auth = auth;
>>>>>>> 5e21ac9a8408cf132a049eb2730d818cf0f8132f:printsystem/auth.js
