...
const settings = {
    ...
    async init() {
        ...
        if (window.location.pathname.includes('/ps/configuracoes/presset/')) {
        ...
        list.innerHTML = this.profiles.map(p => `
            <div class="settings-item">
                ...
                <div class="item-actions">
                    <button class="btn-secondary btn-icon-only" onclick="location.href='/ps/configuracoes/presset/?edit=${p.id}'" title="Editar">
                    ...
                    <button class="btn-secondary btn-icon-only" onclick="location.href='/ps/configuracoes/presset/?clone=${p.id}'" title="Clonar">
    ...
    setupEventListeners() {
        ...
        safeClick('btn-add-profile', () => location.href = '/ps/configuracoes/presset/');
        safeClick('btn-close-profile', () => location.href = '/ps/configuracoes/');
        safeClick('btn-save-profile', () => this.saveProfile());
        safeClick('btn-add-global', () => this.addGlobalCost());
        safeClick('btn-cancel-global', () => this.cancelGlobalEdit());
    ...
    populateProfileEditor(profile, isClone) {
        ...
        document.getElementById('edit-profile-material').value = profile.materialPerWeight.toFixed(2);
        document.getElementById('edit-profile-labor').value = profile.laborPerHour.toFixed(2);
        document.getElementById('edit-profile-energy').value = profile.energyPerHour.toFixed(2);
        document.getElementById('edit-profile-maint').value = profile.maintenancePerHour.toFixed(2);
    ...
    async saveProfile() {
        ...
        try {
            ...
            location.href = '/ps/configuracoes/';
        } catch (err) {
            console.error(err);
            if (btn) {
                btn.disabled = false;
    ...
    editGlobalCost(id) {
        ...
        if (cost) {
            ...
            document.getElementById('new-global-price').value = cost.price.toFixed(2);
            document.getElementById('btn-add-global').innerText = 'Salvar';

            const btnCancel = document.getElementById('btn-cancel-global');
...
window.settings = settings;
>>>>>>> 5e21ac9a8408cf132a049eb2730d818cf0f8132f:printsystem/settings.js
