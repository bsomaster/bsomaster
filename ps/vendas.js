...
const vendas = {
    ...
    updateStats() {
        ...
        if (period === 'margem') {
            ...
            filteredSales = this.sales.filter(s => {
                const d = new Date(s.data_venda || s.data_agendamento || s.data_cancelamento);
    ...
    renderFilteredLists(filteredSales) {
        ...
        const canceladas = filteredSales.filter(s => s.status === 'cancelada')
            .sort((a, b) => new Date(b.data_cancelamento || b.data_venda || b.data_agendamento) - new Date(a.data_cancelamento || a.data_venda || a.data_agendamento));
    ...
    createSaleCard(sale) {
        ...
        div.onclick = () => location.href = `/ps/vendas/item/?id=${sale.id}`;

        let date = null;
        if (sale.status === 'agendada') date = sale.data_agendamento;
        else if (sale.status === 'cancelada') date = sale.data_cancelamento || sale.data_venda || sale.data_agendamento;
        else date = sale.data_venda;
        ...
        div.innerHTML = `
            <div class="sale-card-content">
                <div class="sale-main-info">
                    ...
                    <span class="sale-date">Data: ${formattedDate} - ${formattedTime}</span>
                </div>
            ...
            if (s.status === 'concluida') {
                ...
                tbody.innerHTML += `
                    <tr>
                        ...
                        <td>${new Date(s.data_cancelamento || s.data_venda || s.data_agendamento).toLocaleDateString()}</td>
                        <td>Custo Cancel.</td>
    ...
    initSelectors() {
        ...
        // Init Period
        document.getElementById('stats-period').value = 'margem';
        document.getElementById('stats-period-display').value = 'Margem';
        ...
        document.getElementById('stats-start-date').value = formatDate(firstOfMonth);
        document.getElementById('display-start-date').innerText = firstOfMonth.toLocaleDateString('pt-BR');

        document.getElementById('stats-end-date').value = formatDate(lastOfMonth);
...
window.vendas = vendas;
>>>>>>> 5e21ac9a8408cf132a049eb2730d818cf0f8132f:printsystem/vendas.js
